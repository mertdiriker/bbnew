@extends('dashboards.users.layouts.user-dash-layout')
@section('title','Stok Giris')

@section('content')

<form method="POST" name="frm" action="{{ route('userAddStockEntryCheck') }}">
	@csrf

<table border="2"    id="idarikabul" width="100%" >
  <tr>
    <td align="center" valign=top align=center>
<table border="4"    id="AutoNumber1" >
 <input type="hidden" value="1" name="girdi" >
	<tr>
	<td  rowspan="3">
	 <img src="data:image/png;base64," alt=""></td>
	<td  rowspan="2" colspan="2">
	<p align="center"><b>İDARİ KABUL FORMU</b></td>
	<td ><?php echo \Carbon\Carbon::now(); ?></td>
  </tr>
 	<tr>
	<td height="9" width="208">Depo Giriş No : </td>
  </tr>
 <tr>
	<td height="9" width="880" colspan="4" >
	&nbsp;</td>
  </tr>
 	<tr>
	<td ><b>Yan Sanayi / Müşteri</b></td>

	<td >&nbsp;	@foreach ($entrys as $entry) {{ $entry->firma_Ad }} @endforeach</td>
	<td ><b>Kabul Edilen Ürün Miktarı</b></td>
	<td > 
	<input class="form-control" style="width:100px" Class="text" maxLength="50" size="7" type="number" name="StockItem" value="@foreach ($entrys as $entry) {{ $entry->urundetay_kolimiktar }} @endforeach" required> @foreach ($entrys as $entry) {{ $entry->urun_Olcut }} @endforeach/<input class="form-control" Class="text" type="number" maxLength="3" size="2" style="width:100px" name="StockAdet" value="1" required> 
	Adet</td>
  </tr>
 	<tr>
	<td ><b>Ürün / Malzeme Adı&nbsp; </b> </td>
	<td  style="height:60px;"colspan="3">&nbsp;@foreach ($entrys as $entry) {{ $entry->urun_Ad }} @endforeach <input type="hidden" name="urunid" value="@foreach ($entrys as $entry) {{ $entry->urun_ID }} @endforeach"></td>
  </tr>
 	<tr>
	<td ><b>Ürün / Malzeme No</b></td>
	<td >&nbsp; @foreach ($entrys as $entry) {{ $entry->urun_Kod }} @endforeach</td>	
	<td ><b>İrsaliye Tarihi / <br>İrsaliye No</b></td>
	<td > 
	<input class="form-control" type="date" maxLength="50" size="7" style="width:160px"  name="StockIrsalyDate" value="" required>/ 
	<input class="form-control" type="text" Class="text" maxLength="50" size="12" style="width:160px;"  name="StockIrsalyNo" value="" required></td>
  </tr>
  
 <tr>
	<td height="9" width="100"><b>Ham Malzeme Lot No</b></td>
	<td height="9" width="160">&nbsp;<input class="form-control" Class="text" style="width:160px;" maxLength="50" size="20"  name="StockLotNo" value="" required></td>
	<td height="9" width="160"><b>Önem Sınıfı</b></td>
	<td height="9" width="208">&nbsp;@foreach ($entrys as $entry) {{ $entry->urundetay_onemsinif }} @endforeach</td>

  </tr>
  	
 	<tr>
	<td ><b>Üretim Tarihi</b></td>
	<td >&nbsp;<input class="form-control" type="date" style="width:160px;" Class="text" maxLength="50" size="20"  name="ProductionDate" value="" required></td>
	<td  align="center">
	<p align="left"><b>Depo Konumu</b></td>
	<td align="center">
	<p align="left">
	<input class="form-control" Class="text" style="width:160px;" maxLength="50" size="20"  name="ProductionLocation" value="" required></td>
  </tr>
  	
 	<tr>
	<td  colspan="2"></td>
	<td  align="center"><b>OK</b></td>
	<td align="center"><b>DÜZELTME</b></td>
  </tr>
	<tr>
	<td colspan="2"><b>1 - Paket adeti uygun mu?</b></td>
	<td  align="center"><input class="form-control" type="checkbox" name="ControlSize" value="ON" checked></td>
	<td> <input type="text" class="form-control" style="width:280px;" name="q1"> </td>
  </tr>
	<tr>
	<td  colspan="2"><b>2 - Ambalaj uygun mu?</b></td>
	<td  align="center">
	<input class="form-control" type="checkbox" name="ControlSize0" value="ON" checked></td>
	<td> <input type="text" class="form-control" style="width:280px;" name="q2"> </td>
  </tr>
 	<tr>
	<td  colspan="2"><b>3 - Etiket bilgileri sipariş bilgileri ile uyuşuyor mu?</b></td>
	<td  align="center">
	<input class="form-control" type="checkbox" name="ControlSize1" value="ON" checked></td>
	<td> <input type="text" class="form-control" style="width:280px;" name="q3" > </td>
  </tr>
 	<tr>
	<td  colspan="4"></td>
  </tr>
 	<tr>
	<td colspan="4">*Kontroller Giriş Kontrol Planına göre, İdari Kabul talimatına göre yapılacaktır <br>
					       *Hata durumunda P-03 Uygun olmayan ürünün kontrolü prosedürüne göre işlem yapılır.
 	</td>
  </tr>
	<tr>
	<td  ><b>Kontrol Eden : </b></td>
	<td  colspan="3">&nbsp;<input class="form-control" Class="text" maxLength="50" size="35"  name="StockPersonel" value="<?php echo Auth::user()->name; ?>" required></td>
  </tr>
 	<tr>
	<td ><b>Açıklama</b></td>
	<td  colspan="3">&nbsp;<input class="form-control" Class="text" maxLength="50" size="34"  name="aciklama" value=""></td>
  </tr>
 
 	</table> 	
 
   <p>
   <button type="submit" class="btn btn-success">Gir</button>
  </tr>
  </table>
</form>




@endsection